...
class TestResolveEntityReferences:
    ...
    @patch('musigree.offline.data_access_layer.entity_data_access.EntityDataAccess.get_id_by_entity_type_and_entity_name')
...
class TestResolveReleaseReferences:
    ...
    async def test_resolve_release_references_with_labels(
        ...
        mock_entity_repository.get_entity_id_by_entity_type_and_entity_name.side_effect = [11, 12]
    ...
    async def test_resolve_release_references_with_companies(
        ...
        # Mock the repository method and internal ID conversion
        mock_entity_repository.get_entity_id_by_entity_type_and_entity_name.side_effect = [21, 22]
    ...
    async def test_resolve_release_references_with_both(
        ...
        # Mock the repository method and internal ID conversion
        mock_entity_repository.get_entity_id_by_entity_type_and_entity_name.side_effect = [11, 21]
...
class TestGetIdByEntityTypeAndEntityName:
    ...
    async def test_get_id_cache_miss_db_miss(
        ...
        mock_get_cache.return_value = mock_cache
        mock_cache.get.return_value = None
        mock_entity_repository.get_id_by_entity_type_and_entity_name.side_effect = NotFoundError(message="Entity not found")
        ...
        mock_cache.set.assert_called_once_with("Nonexistent Artist_EntityType.ARTIST", EntityDataAccess.CACHE_ENTRY_IS_NULL)
    ...
    async def test_get_id_cache_miss_db_miss_with_logging(
        self, mock_log: Mock, mock_get_cache: Mock, mock_cache: Mock, mock_entity_repository: AsyncMock
    ) -> None:
...
class TestInitTextSearchIndex:
    ...
    async def test_init_text_search_index_success(
        self, mock_entity_repository: AsyncMock, mock_loader_base: Mock, mock_text_search_index: Mock
    ) -> None:
        ...
        async def mock_all_ids_and_names():
