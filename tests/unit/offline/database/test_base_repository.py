...
class TestBaseRepository:
    ...
    def test_base_repository_init_no_schema_class(self) -> None:
        ...
        with pytest.raises(UnprocessableError, match="Can not initiate the class without schema_class attribute"):
            BadRepository()
    ...
    async def test_count_non_integer_result(self, mock_execute: AsyncMock) -> None:
        ...
        with patch.object(BaseRepository, '_session', new_callable=PropertyMock):
            ...
            # Execute & Verify
            with pytest.raises(UnprocessableError, match="For some reason count function returned not an integer"):
                await repo.count()
    ...
    async def test_save_success(self) -> None:
        ...
        with patch.object(BaseRepository, '_session', new_callable=PropertyMock) as mock_session_prop:
            ...
            with patch.object(repo, 'schema_class', return_value=mock_table_instance) as mock_schema:
                # Execute
    ...
    async def test_save_integrity_error(self) -> None:
        ...
        with patch.object(BaseRepository, '_session', new_callable=PropertyMock) as mock_session_prop:
            ...
            with patch.object(repo, 'schema_class', side_effect=IntegrityError("Integrity error", None, Exception("Original error"))):
                # Execute & Verify
    ...
    async def test_save_invalid_request_error(self) -> None:
        ...
        with patch.object(BaseRepository, '_session', new_callable=PropertyMock) as mock_session_prop:
            ...
            with patch.object(repo, 'schema_class', side_effect=InvalidRequestError("Invalid request")):
                # Execute & Verify
                with pytest.raises(DatabaseError):
                    await repo._save(payload)
    ...
    async def test_update_integrity_error_during_execute(self, mock_execute: AsyncMock) -> None:
        ...
        with patch.object(BaseRepository, '_session', new_callable=PropertyMock):
            repo = TestRepository()
            mock_execute.side_effect = IntegrityError("Integrity error", None, Exception("Original error"))
    ...
    @patch('musigree.offline.database.base_repository.BaseRepository.execute')
    async def test_update_invalid_request_error_during_execute(self, mock_execute: AsyncMock) -> None:
