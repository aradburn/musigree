...
class PermissionsPolicy:
    ...
    def __init__(self, app: ASGIApp, Option: PermissionsPolicyOptions | None = None):
        """
        ...
        if Option is None or not Option:
            ...
            warn(
                "Permission-Policy header is still under working draft, browsers might give warnings and errors",
                SyntaxWarning,
                2,
            )
    ...
    def __PolicyCheck__(self, Option: PermissionsPolicyOptions, Policy: list[str]) -> None:
        ...
        for i, key in enumerate[str](keys):
            if key not in Policy:
                raise SyntaxError(f"The Policy {key} does not exist")

            self.PolicyString += key
            # noinspection PyUnnecessaryCast,PyTypedDict
            values: OptionValues = cast(OptionValues, Option.get(key, []))

            if len(values) != 1:
                if "*" in values:
                    raise SyntaxError("Cannot use wildcard(*) with other allowlist.")
            else:
                if "*" in values:
                    self.PolicyString += "=*, "
                    continue
